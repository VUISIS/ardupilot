...
class ChibiOS::Encryption;

class ChibiOS::EncryptedUARTDriver : public AP_HAL::UARTDriver {
public:
    EncryptedUARTDriver(uint8_t serial_num);
    ~EncryptedUARTDriver();

    /* Do not allow copies */
    EncryptedUARTDriver(const UARTDriver &other) = delete;
    EncryptedUARTDriver &operator=(const UARTDriver&) = delete;
...
private:
    ...
    static ChibiOS::EncryptedUARTDriver *uart_drivers[UART_MAX_DRIVERS];
...
#ifndef HAL_UART_NODMA
    void dma_tx_allocate(ChibiOS::Shared_DMA *ctx);
    void dma_tx_deallocate(ChibiOS::Shared_DMA *ctx);
...
namespace ChibiOS 
{
    typedef EncryptedUARTDriver UARTDriver;
}