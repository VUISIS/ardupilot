...
const EncryptedUARTDriver::SerialDef EncryptedUARTDriver::_serial_tab[] = { HAL_UART_DEVICE_LIST };

// handle for UART handling thread
thread_t* volatile EncryptedUARTDriver::uart_rx_thread_ctx;

// table to find UARTDrivers from serial number, used for event handling
EncryptedUARTDriver *EncryptedUARTDriver::uart_drivers[UART_MAX_DRIVERS];
...
EncryptedUARTDriver::EncryptedUARTDriver(uint8_t _serial_num) :
...
EncryptedUARTDriver::~EncryptedUARTDriver()
{
    delete _enc;
}
...
void EncryptedUARTDriver::uart_thread()
{
...
/*
  ...
  unbuffered_write uarts, and run at 1kHz for general UART handling
 */
void EncryptedUARTDriver::uart_rx_thread(void* arg)
{
 ...
 */
void EncryptedUARTDriver::thread_rx_init(void)
{
 ...
 */
void EncryptedUARTDriver::thread_init(void)
{
...
void EncryptedUARTDriver::uart_thread_trampoline(void* p)
{
    EncryptedUARTDriver* uart = static_cast<EncryptedUARTDriver*>(p);
    uart->uart_thread();
}
...
void EncryptedUARTDriver::begin(uint32_t b, uint16_t rxS, uint16_t txS)
{
    thread_rx_init();
        ...
        if (_baudrate != 0) {
            ...
            if (tx_dma_enabled && dma_handle == nullptr) {
                ...
                dma_handle = new Shared_DMA(sdef.dma_tx_stream_id,
                                            SHARED_DMA_NONE,
                                            FUNCTOR_BIND_MEMBER(&EncryptedUARTDriver::dma_tx_allocate, void, Shared_DMA *),
                                            FUNCTOR_BIND_MEMBER(&EncryptedUARTDriver::dma_tx_deallocate, void, Shared_DMA *));
...
#ifndef HAL_UART_NODMA
void EncryptedUARTDriver::dma_tx_allocate(Shared_DMA *ctx)
{
#if HAL_USE_SERIAL == TRUE
    if (txdma != nullptr) {
        return;
...
#ifndef HAL_UART_NODMA
void EncryptedUARTDriver::dma_rx_enable(void)
{
...
void EncryptedUARTDriver::dma_tx_deallocate(Shared_DMA *ctx)
{
    chSysLock();
...
#ifndef HAL_UART_NODMA
void EncryptedUARTDriver::rx_irq_cb(void* self)
{
#if HAL_USE_SERIAL == TRUE
    EncryptedUARTDriver* uart_drv = (EncryptedUARTDriver*)self;
 ...
 */
__RAMFUNC__ void EncryptedUARTDriver::rxbuff_full_irq(void* self, uint32_t flags)
{
#if HAL_USE_SERIAL == TRUE
    EncryptedUARTDriver* uart_drv = (EncryptedUARTDriver*)self;
    if (!uart_drv->rx_dma_enabled) {
    ...
    if (len > 0) {
         ...
         */
        const uint8_t* dec_msg = _enc->decrypt(uart_drv->rx_bounce_buf[bounce_idx], len);
        uart_drv->_readbuf.write(dec_msg, len);
...
void EncryptedUARTDriver::begin(uint32_t b)
{
    if (lock_write_key != 0) {
        return;
    }
...
void EncryptedUARTDriver::begin_locked(uint32_t b, uint32_t key)
{
...
void EncryptedUARTDriver::flush()
{
    if (sdef.is_usb) {
#ifdef HAVE_USB_SERIAL
...
bool EncryptedUARTDriver::is_initialized()
{
    return _tx_initialised && _rx_initialised;
}

void EncryptedUARTDriver::set_blocking_writes(bool blocking)
{
    _blocking_writes = blocking;
}

bool EncryptedUARTDriver::tx_pending() { return _writebuf.available() > 0; }


/*
    get the requested usb baudrate - 0 = none
*/
uint32_t EncryptedUARTDriver::get_usb_baud() const
{
...
uint32_t EncryptedUARTDriver::available() {
...
uint32_t EncryptedUARTDriver::available_locked(uint32_t key)
{
...
uint32_t EncryptedUARTDriver::txspace()
{
    if (!_tx_initialised) {
        return 0;
    }
...
bool EncryptedUARTDriver::discard_input()
{
...
ssize_t EncryptedUARTDriver::read(uint8_t *buffer, uint16_t count)
{
    if (lock_read_key != 0 || _uart_owner_thd != chThdGetSelfX()){
        return -1;
    }
...
int16_t EncryptedUARTDriver::read()
{
    if (lock_read_key != 0 || _uart_owner_thd != chThdGetSelfX()){
        return -1;
    }
...
int16_t EncryptedUARTDriver::read_locked(uint32_t key)
{
    if (lock_read_key != 0 && key != lock_read_key) {
        return -1;
    }
...
size_t EncryptedUARTDriver::write(uint8_t c)
{
    if (lock_write_key != 0) {
        return 0;
    }
...
size_t EncryptedUARTDriver::write(const uint8_t *buffer, size_t size)
{
 ...
 */
bool EncryptedUARTDriver::lock_port(uint32_t write_key, uint32_t read_key)
{
...
*/
size_t EncryptedUARTDriver::write_locked(const uint8_t *buffer, size_t size, uint32_t key)
{
    if (lock_write_key != 0 && key != lock_write_key) {
        return 0;
    }
 ...
 */
bool EncryptedUARTDriver::wait_timeout(uint16_t n, uint32_t timeout_ms)
{
 ...
 */
__RAMFUNC__ void EncryptedUARTDriver::tx_complete(void* self, uint32_t flags)
{
    EncryptedUARTDriver* uart_drv = (EncryptedUARTDriver*)self;
    chSysLockFromISR();
 ...
 */
void EncryptedUARTDriver::write_pending_bytes_DMA(uint32_t n)
{
    // sanity check
    if (!dma_handle) {
        return;
    ...
    while (n > 0) {
...

        {
            ...
            const uint8_t* enc_buf = _enc->encrypt(tx_bounce_buf, (size_t)tx_len);
            if (tx_len == 0) {
                return; // all done
            }
        ...
        stm32_cacheBufferFlush(enc_buf, tx_len);
        dmaStreamSetMemory0(txdma, enc_buf);
 ...
 */
void EncryptedUARTDriver::write_pending_bytes_NODMA(uint32_t n)
{
    WITH_SEMAPHORE(_write_mutex);
    ...
    for (int i = 0; i < n_vec; i++) {
        int ret = -1;
        const uint8_t* enc_buffer = _enc->encrypt(vec[i].data, vec[i].len);
        if (sdef.is_usb) {
            ret = 0;
#ifdef HAVE_USB_SERIAL
            ret = chnWriteTimeout((SerialUSBDriver*)sdef.serial, enc_buffer, vec[i].len, TIME_IMMEDIATE);
#endif
        } else {
#if HAL_USE_SERIAL == TRUE
            ret = chnWriteTimeout((SerialDriver*)sdef.serial, enc_buffer, vec[i].len, TIME_IMMEDIATE);
#endif
        }
        if (ret < 0) {
            break;
...
#pragma GCC diagnostic push
#pragma GCC diagnostic error "-Wframe-larger-than=128"
void EncryptedUARTDriver::write_pending_bytes(void)
{
 ...
 */
void EncryptedUARTDriver::half_duplex_setup_tx(void)
{
#ifdef HAVE_USB_SERIAL
    if (!hd_tx_active) {
 ...
 */
void EncryptedUARTDriver::_rx_timer_tick(void)
{
    ...
    if (rx_dma_enabled && rxdma) {
        ...
        if (!enabled) {
            ...
            if (len != 0) {
                const uint8_t* dec_buf = _enc->decrypt(rx_bounce_buf[rx_bounce_idx], len);
                _readbuf.write(dec_buf, len);
                _rx_stats_bytes += len;
...
// regular serial read
void EncryptedUARTDriver::read_bytes_NODMA()
{
    ...
    for (int i = 0; i < n_vec; i++) {
        int ret = 0;
        uint8_t* msg = new uint8_t[vec[i].len];
        //Do a non-blocking read
        if (sdef.is_usb) {
#ifdef HAVE_USB_SERIAL
            ret = chnReadTimeout((SerialUSBDriver*)sdef.serial, msg, vec[i].len, TIME_IMMEDIATE);
#endif
        } else {
#if HAL_USE_SERIAL == TRUE
            ret = chnReadTimeout((SerialDriver*)sdef.serial, msg, vec[i].len, TIME_IMMEDIATE);
#endif
        }
        vec[i].data = _enc->decrypt(msg, vec[i].len);
        if (ret < 0) {
            break;
        }
...
/*
  ...
  overhead in the main task enormously.
 */
void EncryptedUARTDriver::_tx_timer_tick(void)
{
    if (!_tx_initialised) {
        return;
    }
 ...
 */
void EncryptedUARTDriver::set_flow_control(enum flow_control flowcontrol)
{
    if (sdef.is_usb) {
 ...
 */
__RAMFUNC__ void EncryptedUARTDriver::update_rts_line(void)
{
 ...
 */
bool EncryptedUARTDriver::set_unbuffered_writes(bool on)
{
...
/*
  setup parity
 */
void EncryptedUARTDriver::configure_parity(uint8_t v)
{
    if (sdef.is_usb) {
        // not possible
        return;
...
/*
  set stop bits
 */
void EncryptedUARTDriver::set_stop_bits(int n)
{
    if (sdef.is_usb) {
        // not possible
        return;
...
__RAMFUNC__ void EncryptedUARTDriver::receive_timestamp_update(void)
{
...
*/
uint64_t EncryptedUARTDriver::receive_time_constraint_us(uint16_t nbytes)
{
...
*/
void EncryptedUARTDriver::set_pushpull(uint16_t options)
{
...
bool EncryptedUARTDriver::set_options(uint16_t options)
{
    if (sdef.is_usb) {
...
uint16_t EncryptedUARTDriver::get_options(void) const
{
...
void EncryptedUARTDriver::uart_info(ExpandingString &str)
{
...
/*
  ...
  not available
*/
bool EncryptedUARTDriver::set_CTS_pin(bool high)
{
    if (_flow_control != FLOW_CONTROL_DISABLE) {
...
/*
  ...
  not available
*/
bool EncryptedUARTDriver::set_RTS_pin(bool high)
{
    if (_flow_control != FLOW_CONTROL_DISABLE) {
...
/*
  initialise the USB bus, called from both EncryptedUARTDriver and stdio for startup debug
...
void EncryptedUARTDriver::disable_rxtx(void) const
{
    if (arx_line) {
